class Projects::DashboardsController < Projects::ApplicationController
  ...
  def show
    ...
    @sprints = @project.sprints.includes(:issues).where( archived: false ).where('issues_count > 0').order('id DESC').limit(10)
    @issues  = @project.issues.includes(:user, :assignee, :labels).joins(:participations)
