  ...
  describe '#process' do
    it 'replaces all mentioned resource with a link' do
      ...
      content, mentioned = MentionService.parse_mentioned(
          ...
          "##{@issues[0].sequential_id}, ^#{@polls[0].sequential_id} and !#{@sprints[0].sequential_id}", @project)
      expect(content).to eq("This is a test comment which references #{link_user}, #{link_attachment}, #{link_issue}, #{link_poll} and #{link_sprint}")
    end
